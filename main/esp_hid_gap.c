...
static void bt_gap_event_handler(esp_bt_gap_cb_event_t event, esp_bt_gap_cb_param_t *param)
{
    ...
    case ESP_BT_GAP_AUTH_CMPL_EVT: {
      ESP_LOGI(TAG, "BT GAP_AUTH_CMPL_EVT status:%d", param->auth_cmpl.stat);
      
      break;
    }
    case ESP_BT_GAP_PIN_REQ_EVT: {
      ESP_LOGI(TAG, "BT GAP_PIN_REQ_EVT min_16_digit:%d", param->pin_req.min_16_digit);
#define PIN_LENGTH 6
      const uint8_t *pin_code = esp_bt_dev_get_address();
      const uint8_t pin_code_r[] = {
	pin_code[5], pin_code[4], pin_code[3], pin_code[2], pin_code[1], pin_code[0]
      };
      //uint8_t pin_code[PIN_LENGTH] = {0x59, 0x33, 0x53, 0x4C, 0x22, 0x00};
      fprintf(stderr, "BDA: 0x");
      for(int i = 0; i < ESP_BD_ADDR_LEN; i++) {
	fprintf(stderr, "%02X", param->pin_req.bda[i]);
      };
      fprintf(stderr, "\n");
      esp_bt_gap_pin_reply(param->pin_req.bda, true, PIN_LENGTH, pin_code_r);
      ESP_LOGI(TAG, "BT ADDR %02X:%02X:%02X:%02X:%02X:%02X",
	       pin_code_r[0], pin_code_r[1], pin_code_r[2], pin_code_r[3],
	       pin_code_r[4], pin_code_r[5]);
      break;
    }
    default:
